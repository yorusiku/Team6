...
export default class CustomerSearch extends NavigationMixin(LightningElement) {
    ...
    handleRowAction(event) {
        ...
        if (action.name === 'generateVOC') {
            this.navigateToVOCRecordPage(row.Id, row.Contact_Products__r.Contact__c); 
            console.log('이거 뭐니 : '+row.Contact_Products__r.Contact__c);
        }
    }

    // VOC__c 레코드 생성 페이지로 이동
    navigateToVOCRecordPage(salesProductId, contactName) {
        const valueGood = `Sales_Product__c=${salesProductId},CustomerName__c=${contactName}`;
